...
def cashier_manager_required(func):
    @wraps(func)
    def wrapper(request, *args, **kwargs):
        membership = request.user.membership_set.filter(organization=request.org).first()
        if not membership or not membership.cashier_manager:
            raise PermissionDenied("Sie haben keine Berechtigung für diese Aktion.")
        return func(request, *args, **kwargs)
    return wrapper


def leiterrundenmitglied_required(func):
    @wraps(func)
    def wrapper(request, *args, **kwargs):
        membership = request.user.membership_set.filter(organization=request.org).first()
...
def pro5_required(func):
    @wraps(func)
    def wrapper(request, *args, **kwargs):
        org = getattr(request, "org", None)
        if not org or not org.pro5:
            raise PermissionDenied("Diese Organisation hat keine Pro5-Berechtigung.")
        return func(request, *args, **kwargs)
    return wrapper


def pro6_required(func):
    @wraps(func)
    def wrapper(request, *args, **kwargs):
        org = getattr(request, "org", None)
        if not org or not org.pro6:
            raise PermissionDenied("Diese Organisation hat keine Pro6-Berechtigung.")
        return func(request, *args, **kwargs)
    return wrapper
